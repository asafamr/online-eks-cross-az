...
class CrossAzLogger:
    def __init__(self,accumulation_minutes=5, quite=False, cluster_name=None, verbose=False,output_file_name='./cross-az.csv', cf_stack_name='online-eks-cross-az') -> None:
    ...
    def get_pods_mapping(self):
        self.log('gathering pod and node data...', is_debug=True)
        AZ_LABEL = "topology.kubernetes.io/zone"
        TIME_DATE_FORMAT = "%Y-%m-%dT%H:%M:%SZ"

        nodes_azs = {}

        self.log('listing nodes...', is_debug=True)
        ...
        self.log(f'got {len(nodes.items)} nodes', is_debug=True)

        pods_info = []

        self.log('listing pods...', is_debug=True)
        pods = self.kube_client.list_pod_for_all_namespaces(label_selector="app", watch=False)
        self.log(f'got {len(pods.items)} pods', is_debug=True)
    ...
    def test_credentials(self):
        ...
        try:
            ...
            _flow_logs = self.ec2_client.describe_flow_logs(Filter=[
                {
                    'Name': 'resource-id',
                    'Values': [
                        vpc_id,
            ...
            self.log('could not list flow logs from EKS cluster: {e}')
            return False
    ...
    def upload_pod_ips(self, bucket_name):
        ...
        self.log(f"uploading pods mapping csv...", is_debug=True)
        buffer = StringIO()
    ...
    def run(self):
        ...
        self.log("verifying AWS and EKS credentials...")
        ...
        try:
            ...
            with yaspin(text="deploying flow logs and buckets stack...") as sp:
            ...
            self.log(f"accumulating {self.accumulation_minutes} minutes of flow log data...")
            with yaspin(text="elapsed time", timer=True) as sp:
                sleep(self.accumulation_minutes*60)
                sp.ok()

            with yaspin(text="aggregating data in athena...") as sp:
                self.run_athena_named_query_to_csv(logical_physical_mapping['AthenaAnalyzerquerycrossaztrafficbyapp659310A2'],
                                                logical_physical_mapping['AthenaAnalyzerathenaresults88B510AC'],self.output_file_name)
                sp.ok()

            with yaspin(text="cleaning up cloudformation...") as sp:
                self.cleanup_and_delete_cf_stack(self.cf_stack_name)
                sp.ok()

        except KeyboardInterrupt as e:
            self.log(f"keyboard interruption. aborting and trying to clean up...")
            self.cleanup_and_delete_cf_stack(self.cf_stack_name)

        except Exception as e:
            self.log(f"exception while accumulating flow logs: {e}")

            self.log(f"cleaning up cloudformation...")
            self.cleanup_and_delete_cf_stack(self.cf_stack_name)
        ...
        while query_status == 'RUNNING':
            try:
                ...
                if query_status == 'FAILED':
                    raise Exception('Athena query failed to run with error: ' + status_response['QueryExecution']['Status']['StateChangeReason'])
    ...
    def get_named_queries_ids_in_stack(self, stack_name):
        resources=self.cloudformation_client.describe_stack_resources(StackName=stack_name)
        return [x['PhysicalResourceId'] for x in resources['StackResources'] if x.get('ResourceType') == 'AWS::Athena::NamedQuery']

    def run_athena_named_query_to_csv(self, named_query_id, result_bucket, csv_path):
        ...
        response = self.athena_client.start_query_execution(
            ...
            ResultConfiguration={
                'OutputLocation': f's3://{result_bucket}/' 
            },
            QueryExecutionContext={
                'Database': 'eks-inter-az-visibility'
            },
        )
    ...
    def cleanup_and_delete_cf_stack(self, stack_name):
        """
        removes everything but buckets, empties them, then remove them too
        """
    ...
    def deploy_cf_stack(self, cf_spec, stack_name, cf_params={}):
        ...
        if not stack_exists:
            ...
            self.cloudformation_client.create_stack(
                ...
                OnFailure='DELETE',
                Parameters=[
                    {"ParameterKey": k, "ParameterValue": v} for k,v in cf_params.items()
                ],
            )
...
def get_csv():
    ...
    cluster_name = cluster_arn.split("/")[-1]
    # try:
    cluster = eks_client.describe_cluster(name=cluster_name+'sss')
    # except:
    #     breakpoint()
    vpc_id = cluster["cluster"]["resourcesVpcConfig"]["vpcId"]
    ...
    if status is None:
        response = cloud_formation_client.create_stack(
            ...
            OnFailure='DELETE',
            Parameters=[
                {"ParameterKey": "eksClusterName", "ParameterValue": "minutecloud-qa"},
                {"ParameterKey": "eksVpcId", "ParameterValue": "vpc-02b5cc43974aeb0c2"}
            ],
        )
        ...
        try:
            ...
            waiter = cloud_formation_client.get_waiter('stack_update_complete')
            waiter.wait(StackName=stack_name)
        except ClientError as e:
            if 'No updates are to be performed' not in str(e):
                raise e
    ...
    for resource in list(stack["Resources"].keys()):
        if stack["Resources"][resource]['Type'] == 'AWS::EC2::FlowLog':
            del stack["Resources"][resource]

    print('Stopping flow logging...')
    response = cloud_formation_client.update_stack(
        StackName=stack_name,
        TemplateBody=json.dumps(stack),
        Parameters=[
    ...
    response = cloud_formation_client.describe_stack_resource(
            StackName=stack_name,
            LogicalResourceId='FlowLogsvpcflowlogsbucket8625B84A'
        )
    ...
    print('Flow logs cleanup...')
    response = cloud_formation_client.delete_stack(
        StackName=stack_name,
    )

    print('Deleting stack...')
    waiter = cloud_formation_client.get_waiter('stack_delete_complete')
    waiter.wait(StackName=stack_name)
